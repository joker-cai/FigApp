function FigDot12Yi(app,SM,SLW,sz,SMA,myco)
%% FIGLINEYI
...
if ~isnan(iNumberVector)
            ...
            for iMyAxes = mark
                ...
                for j=1:MyDraw
                    if contains(ML,'Scatter')
                        ...
                        if isempty(z)
                            s = scatter(x,y,sz*36,1:length(x),"filled");
                            s.Marker = SM;
                            s.LineWidth = SLW;
                            ...
                            s = scatter3(x,y,z,sz*36,1:length(x),"filled");
                            s.Marker = SM;
                            s.LineWidth = SLW;
                            s.MarkerEdgeAlpha = SMA;
                            s.MarkerFaceAlpha = SMA;
            ...
            for iMyAxes = 1:length(axs)
                ...
                for j=1:MyDraw
                    if contains(ML,'Scatter')
                        ...
                        if isempty(z1)
                            s = scatter(x1,y1,sz*36,1:length(x1),"filled");
                            s.Marker = SM;
                            s.LineWidth = SLW;
                            s.MarkerEdgeAlpha = SMA;
                            s.MarkerFaceAlpha = SMA;
                            ...
                            s = scatter3(x1,y1,z1,sz*36,1:length(x1),"filled");
                            s.Marker = SM;
                            s.LineWidth = SLW;
                            s.MarkerEdgeAlpha = SMA;
                            s.MarkerFaceAlpha = SMA;
    ...
    if ismember(SM,flag)
        f = app.UIFigure; %建立图窗
        message = sprintf('选择错误，请您选择可填充的标记');
